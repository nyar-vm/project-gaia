...
/// Mach-O 可执行文件延迟读取器
#[derive(Debug)]
pub struct ExecutableReader<R: Read + Seek> {
    reader: RefCell<BinaryReader<R, LittleEndian>>,
    config: MachoReadConfig,
    program: OnceLock<Result<MachoProgram, GaiaError>>,
    info: OnceLock<Result<ExecutableInfo, GaiaError>>,
}

impl<R: Read + Seek> ExecutableReader<R> {
    /// 创建一个新的可执行文件读取器
    pub fn new(reader: BinaryReader<R, LittleEndian>, config: MachoReadConfig) -> Self {
        Self { reader: RefCell::new(reader), config, program: OnceLock::new(), info: OnceLock::new() }
    }

    /// 获取程序信息（延迟加载）
    pub fn get_program(&self) -> Result<&MachoProgram, &GaiaError> {
        self.program.get_or_init(|| self.read_program_internal()).as_ref()
    }

    /// 获取文件信息（延迟加载）
    pub fn get_info(&self) -> Result<&ExecutableInfo, &GaiaError> {
        self.info.get_or_init(|| self.read_info()).as_ref()
    }
    ...
    fn read_program_internal(&self) -> Result<MachoProgram, GaiaError> {
        ...
        let header = crate::types::MachoHeader { magic, cpu_type, cpu_subtype, file_type, ncmds, sizeofcmds, flags, reserved };
        ...
        for _ in 0..ncmds {
            ...
            load_commands.push(crate::types::LoadCommand { cmd, cmdsize, data });
        }

        Ok(MachoProgram { header, load_commands, segments: Vec::new(), sections: Vec::new() })
    }

    /// 读取文件基本信息
    fn read_info(&self) -> Result<ExecutableInfo, GaiaError> {
        let mut reader = self.reader.borrow_mut();
        ...
        Ok(ExecutableInfo {
            ...
            sizeofcmds,
            flags,
            entry_point: None, // TODO: 从 LC_MAIN 或 LC_UNIXTHREAD 中提取
        })
    }
}

impl<R: Read + Seek> MachoReader<R> for ExecutableReader<R> {
