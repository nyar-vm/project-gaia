...
pub struct ObjectReader<R: Read + Seek> {
    ...
    info: OnceLock<Result<ObjectInfo, GaiaError>>,
}
...
impl<R: Read + Seek> ObjectReader<R> {
    ...
    /// 获取文件信息（延迟加载）
    pub fn get_info(&self) -> Result<&ObjectInfo, &GaiaError> {
    ...
    /// 读取文件基本信息
    fn read_info(&self) -> Result<ObjectInfo, GaiaError> {
        let mut reader = self.reader.borrow_mut();
        ...
        Ok(ObjectInfo {
            ...
            sizeofcmds,
            flags,
            symbol_count: None, // TODO: 从 LC_SYMTAB 中提取
        })
    }
}

impl<R: Read + Seek> MachoReader<R> for ObjectReader<R> {
