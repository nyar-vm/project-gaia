...
mod tests {
    ...
    fn test_load_command_size_calculation() {
        ...
        // 添加一个4字节数据的加载命令
        let program = builder.add_load_command(0x1, vec![0x00, 0x01, 0x02, 0x03]).build().expect("Failed to build");
    ...
    fn test_multiple_architectures() {
        let architectures = vec![CpuType::X86_64, CpuType::Arm64, CpuType::I386, CpuType::Arm];
