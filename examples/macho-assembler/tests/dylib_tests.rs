...
mod tests {
    ...
    use macho_assembler::{
        ...
        formats::dylib::{DylibReader, DylibWriter},
        helpers::MachoWriter,
    ...
    fn test_dylib_write_read_roundtrip() {
        ...
        let original_program =
            builder.add_load_command(0x1, vec![0x00, 0x01, 0x02, 0x03]).build().expect("Failed to build dylib");
    ...
    fn test_dylib_lazy_loading() {
        ...
        let program = builder.add_load_command(0x2, vec![0x10, 0x20, 0x30, 0x40]).build().expect("Failed to build dylib");
