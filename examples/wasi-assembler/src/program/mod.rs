...
impl WasiProgram {
    ...
    /// 将程序转换为 WAT AST
    #[cfg(feature = "wat")]
    pub fn to_wat(&self) -> gaia_types::Result<oak_wat::ast::WatRoot> {
        use oak_wat::ast::{
            WatExport, WatFunc, WatImport, WatInstruction, WatItem, WatLocal, WatModule, WatModuleField, WatParam, WatResult,
            WatRoot, WatTypeKind,
        };
        ...
        for (i, import) in self.imports.iter().enumerate() {
            let kind = match import.import_type {
                WasmImportType::Function { .. } => "func",
                WasmImportType::Table { .. } => "table",
                WasmImportType::Memory { .. } => "memory",
                WasmImportType::Global { .. } => "global",
            };
            let index = match import.import_type {
                WasmImportType::Function { type_index } => type_index,
        ...
        for (i, func) in self.functions.iter().enumerate() {
            let func_type = &self.function_types[func.type_index as usize];

            let mut params = Vec::new();
            for (j, param) in func_type.params.iter().enumerate() {
                params.push(WatParam {
                    name: Some(format!("$p{}", j)),
                    ty: match param {
                        WasmValueType::I32 => WatTypeKind::I32,
                        WasmValueType::I64 => WatTypeKind::I64,
                        WasmValueType::F32 => WatTypeKind::F32,
                        WasmValueType::F64 => WatTypeKind::F64,
                        _ => WatTypeKind::I32, // Fallback
                    },
                });
            }
            ...
            let mut local_idx = 0;
            for local_group in &func.locals {
                for _ in 0..local_group.count {
                    locals.push(WatLocal {
                        name: Some(format!("$l{}", local_idx)),
                        ty: match local_group.value_type {
                            WasmValueType::I32 => WatTypeKind::I32,
                            WasmValueType::I64 => WatTypeKind::I64,
                            WasmValueType::F32 => WatTypeKind::F32,
                            WasmValueType::F64 => WatTypeKind::F64,
            ...
            for instr in &func.body {
                let instr_str = match instr {
                    WasiInstruction::Nop => "nop".to_string(),
                    WasiInstruction::Unreachable => "unreachable".to_string(),
                    WasiInstruction::Return => "return".to_string(),
                    WasiInstruction::Drop => "drop".to_string(),
                    WasiInstruction::Select => "select".to_string(),
                    WasiInstruction::I32Add => "i32.add".to_string(),
                    WasiInstruction::I32Sub => "i32.sub".to_string(),
                    WasiInstruction::I32Mul => "i32.mul".to_string(),
                    WasiInstruction::I32DivS => "i32.div_s".to_string(),
                    WasiInstruction::I32DivU => "i32.div_u".to_string(),
                    WasiInstruction::I32RemS => "i32.rem_s".to_string(),
                    WasiInstruction::I32RemU => "i32.rem_u".to_string(),
                    WasiInstruction::I32And => "i32.and".to_string(),
                    WasiInstruction::I32Or => "i32.or".to_string(),
                    WasiInstruction::I32Xor => "i32.xor".to_string(),
                    WasiInstruction::I32Shl => "i32.shl".to_string(),
                    WasiInstruction::I32ShrS => "i32.shr_s".to_string(),
                    WasiInstruction::I32ShrU => "i32.shr_u".to_string(),
                    WasiInstruction::I32Rotl => "i32.rotl".to_string(),
                    WasiInstruction::I32Rotr => "i32.rotr".to_string(),
                    WasiInstruction::I32Eqz => "i32.eqz".to_string(),
                    WasiInstruction::I32Eq => "i32.eq".to_string(),
                    WasiInstruction::I32Ne => "i32.ne".to_string(),
                    WasiInstruction::I32LtS => "i32.lt_s".to_string(),
                    WasiInstruction::I32LtU => "i32.lt_u".to_string(),
                    WasiInstruction::I32GtS => "i32.gt_s".to_string(),
                    WasiInstruction::I32GtU => "i32.gt_u".to_string(),
                    WasiInstruction::I32LeS => "i32.le_s".to_string(),
                    WasiInstruction::I32LeU => "i32.le_u".to_string(),
                    WasiInstruction::I32GeS => "i32.ge_s".to_string(),
                    WasiInstruction::I32GeU => "i32.ge_u".to_string(),
                    WasiInstruction::I32Load { offset, align } => format!("i32.load offset={} align={}", offset, align),
                    WasiInstruction::I64Load { offset, align } => format!("i64.load offset={} align={}", offset, align),
                    WasiInstruction::F32Load { offset, align } => format!("f32.load offset={} align={}", offset, align),
                    WasiInstruction::F64Load { offset, align } => format!("f64.load offset={} align={}", offset, align),
                    WasiInstruction::I32Store { offset, align } => format!("i32.store offset={} align={}", offset, align),
                    WasiInstruction::I64Store { offset, align } => format!("i64.store offset={} align={}", offset, align),
                    WasiInstruction::F32Store { offset, align } => format!("f32.store offset={} align={}", offset, align),
                    WasiInstruction::F64Store { offset, align } => format!("f64.store offset={} align={}", offset, align),
                    WasiInstruction::StructNew { type_index } => format!("struct.new {}", type_index),
                    WasiInstruction::StructGet { type_index, field_index } => {
                        format!("struct.get {} {}", type_index, field_index)
                    }
                    WasiInstruction::StructSet { type_index, field_index } => {
                        format!("struct.set {} {}", type_index, field_index)
                    }
                    WasiInstruction::TaskBackpressure => "task.backpressure".to_string(),
                    WasiInstruction::TaskReturn => "task.return".to_string(),
                    WasiInstruction::TaskWait => "task.wait".to_string(),
                    WasiInstruction::TaskPoll => "task.poll".to_string(),
                    WasiInstruction::TaskYield => "task.yield".to_string(),
                    WasiInstruction::ErrorContextNew => "error_context.new".to_string(),
                    WasiInstruction::ErrorContextDebugMessage => "error_context.debug_message".to_string(),
                    WasiInstruction::LocalGet { local_index } => format!("local.get {}", local_index),
                    WasiInstruction::LocalSet { local_index } => format!("local.set {}", local_index),
                    WasiInstruction::I32Const { value } => format!("i32.const {}", value),
                    WasiInstruction::I64Const { value } => format!("i64.const {}", value),
                    WasiInstruction::F32Const { value } => format!("f32.const {}", value),
                    WasiInstruction::F64Const { value } => format!("f64.const {}", value),
                    WasiInstruction::Call { function_index } => format!("call {}", function_index),
                    WasiInstruction::Block { .. } => "block".to_string(),
                    WasiInstruction::Loop { .. } => "loop".to_string(),
                    WasiInstruction::If { .. } => "if".to_string(),
                    WasiInstruction::Else => "else".to_string(),
                    WasiInstruction::End => "end".to_string(),
                    WasiInstruction::Br { label_index } => format!("br {}", label_index),
        ...
        // 3. 写入导出
        for export in &self.exports {
            let (kind, index) = match export.export_type {
                WasmExportType::Function { function_index } => ("func", function_index),
                WasmExportType::Table { table_index } => ("table", table_index),
        ...
        if self.program_type == WasiProgramType::CoreModule {
            let module = WatModule { name: self.name.clone(), items: module_items };
            items.push(WatItem::Module(module));
        }
    ...
    pub fn to_wasm(&self) -> Result<Vec<u8>> {
        let mut writer = crate::formats::wasm::writer::WasmWriter::new(Vec::new());
        writer.write(self.clone()).result
    }
}
