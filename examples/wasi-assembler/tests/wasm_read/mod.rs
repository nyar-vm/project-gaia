...
fn compile_wat_to_wasm(_wat_content: &str, output_path: &Path) -> Result<(), GaiaError> {
...
fn read_wasm(path: &Path) -> Result<WasmExpected, GaiaError> {
    ...
    // 提取导出信息
    let exports: Vec<ExportInfo> = prog
        .exports
        .iter()
        .map(|export| {
            let (export_type, index) = match export.export_type {
                wasi_assembler::program::WasmExportType::Function { function_index } => {
                    ("Function".to_string(), function_index)
                }
            ...
            ExportInfo { name: export.name.clone(), export_type, index }
        })
        .collect();

    // 提取导入信息
    let imports: Vec<ImportInfo> = prog
        .imports
        .iter()
        .map(|import| {
            ...
            ImportInfo { module: import.module.clone(), field: import.field.clone(), import_type }
        })
        .collect();
