use gaia_types::{helpers::Url, GaiaError, GaiaErrorKind};
use std::{
    fs,
    path::{Path, PathBuf},
};
use wasi_assembler::{
    ...
    program::{
        WasiExport, WasiFunction, WasiFunctionType, WasiImport, WasiInstruction, WasiMemory, WasiProgramBuilder,
        WasiProgramType, WasmExportType, WasmImportType, WasmMemoryType, WasmValueType,
    },
};
...
fn hello_world() -> Result<(), GaiaError> {
    ...
    let mut program =
        WasiProgramBuilder::new(WasiProgramType::CoreModule).with_name("hello_world").build().expect("Failed to build program");

    // 添加内存
    let memory = WasiMemory { memory_type: WasmMemoryType { min: 1, max: None } };
    ...
    let fd_write_import = WasiImport {
        ...
        import_type: WasmImportType::Function { type_index: fd_write_type_index },
    };
    program.add_import(fd_write_import);

    // 添加 WASI 导入 - proc_exit
    let proc_exit_func_type = WasiFunctionType { params: vec![WasmValueType::I32], results: vec![] };
    ...
    let proc_exit_import = WasiImport {
        ...
        import_type: WasmImportType::Function { type_index: proc_exit_type_index },
    };
    program.add_import(proc_exit_import);

    // 添加主函数类型
    let main_func_type = WasiFunctionType { params: vec![], results: vec![] };
    ...
    // 添加导出
    let main_export = WasiExport { name: "_start".to_string(), export_type: WasmExportType::Function { function_index: 0 } };
    ...
    let absolute_output_path = output_path_buf.canonicalize().map_err(|e| {
        GaiaError::io_error(
            e,
            gaia_types::helpers::url_from_path(&output_path_buf)
                .unwrap_or_else(|_| Url::parse("file:///invalid_path").unwrap()),
        )
    })?;
