...
fn test_wasi_program_to_wat_and_run() {
    ...
    use wasi_assembler::program::{
        WasiExport, WasiFunction, WasiFunctionType, WasiInstruction, WasiProgram, WasiProgramType, WasmExportType,
    };

    // 1. 使用 WasiProgramBuilder 创建一个简单的 WASI 程序
    let func_type = WasiFunctionType { params: vec![], results: vec![] };

    let func_main = WasiFunction {
        type_index: 0,
        locals: vec![],
        body: vec![WasiInstruction::I32Const { value: 42 }, WasiInstruction::Drop],
    };

    let export_main = WasiExport { name: "_start".to_string(), export_type: WasmExportType::Function { function_index: 0 } };
    ...
    let wat_root = program.to_wat().unwrap();
    let wat_content = wat_root.to_source_string();
