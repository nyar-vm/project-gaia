...
pub enum ClrInstruction {
    ...
    /// 带 64 位立即数的指令
    WithImmediate64 { opcode: ClrOpcode, value: i64 },
    /// 带 32 位浮点数的指令
    WithFloat32 { opcode: ClrOpcode, value: f32 },
    /// 带 64 位浮点数的指令
    WithFloat64 { opcode: ClrOpcode, value: f64 },
    /// 带局部变量索引的指令
...
impl ClrProgram {
    ...
    /// 转换为 MSIL AST
    pub fn to_msil(&self) -> msil::MsilRoot {
        let mut items = Vec::new();

        // 添加外部程序集引用
        for ext in &self.external_assemblies {
            items.push(msil::Item::AssemblyExtern(ext.name.clone()));
        }

        // 添加程序集定义
        items.push(msil::Item::Assembly(msil::Assembly { name: self.name.clone(), span: (0..0usize).into() }));

        // 添加模块定义
        if let Some(module) = &self.module {
            items.push(msil::Item::Module(module.name.clone()));
        }

        // 添加类型定义
        for clr_type in &self.types {
            items.push(msil::Item::Class(clr_type.to_msil()));
        }
...
impl ClrType {
    fn to_msil(&self) -> msil::Class {
        msil::Class {
            name: self.name.clone(),
            methods: self.methods.iter().map(|m| m.to_msil()).collect(),
            span: (0..0usize).into(),
        }
    }
}
...
impl ClrMethod {
    fn to_msil(&self) -> msil::Method {
        msil::Method {
            name: self.name.clone(),
            instructions: self.instructions.iter().map(|i| i.to_msil()).collect(),
            span: (0..0usize).into(),
        }
    }
}
...
impl ClrInstruction {
    fn to_msil(&self) -> msil::Instruction {
        match self {
            ClrInstruction::Simple { opcode } => msil::Instruction::Simple(format!("{:?}", opcode).to_lowercase()),
            ClrInstruction::WithString { value, .. } => msil::Instruction::String(value.clone()),
            ClrInstruction::WithMethod { method_ref, .. } => msil::Instruction::Call(method_ref.clone()),
            ClrInstruction::WithImmediate { opcode, value } => {
                msil::Instruction::Simple(format!("{:?} {}", opcode, value).to_lowercase())
            }
            ClrInstruction::WithImmediate64 { opcode, value } => {
                msil::Instruction::Simple(format!("{:?} {}", opcode, value).to_lowercase())
            }
            ClrInstruction::WithFloat32 { opcode, value } => {
                msil::Instruction::Simple(format!("{:?} {}", opcode, value).to_lowercase())
            }
            ClrInstruction::WithFloat64 { opcode, value } => {
                msil::Instruction::Simple(format!("{:?} {}", opcode, value).to_lowercase())
            }
            ClrInstruction::WithLocalVar { opcode, index } => {
                msil::Instruction::Simple(format!("{:?} {}", opcode, index).to_lowercase())
            }
            ClrInstruction::WithParameter { opcode, index } => {
                msil::Instruction::Simple(format!("{:?} {}", opcode, index).to_lowercase())
            }
            ClrInstruction::WithField { opcode, field_ref } => {
                msil::Instruction::Simple(format!("{:?} {}", opcode, field_ref).to_lowercase())
            }
            ClrInstruction::WithType { opcode, type_ref } => {
                msil::Instruction::Simple(format!("{:?} {}", opcode, type_ref).to_lowercase())
            }
            ClrInstruction::WithLabel { opcode, label } => {
                msil::Instruction::Simple(format!("{:?} {}", opcode, label).to_lowercase())
            }
            ClrInstruction::WithSwitch { opcode, labels } => {
                msil::Instruction::Simple(format!("{:?} {}", opcode, labels.join(", ")).to_lowercase())
            }
        }
    }
}
