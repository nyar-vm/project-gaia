...
impl<W: Write + Seek> DotNetWriter<W> {
    ...
    fn write_clr_header_with_offsets(
        ...
        // 将 CLR 头写入到缓冲区的开始位置
        let mut cursor = std::io::Cursor::new(&mut buffer[0..72]);
    ...
    fn calculate_instruction_size(&self, instruction: &ClrInstruction) -> Result<u32, GaiaError> {
        match instruction {
            ...
            ClrInstruction::WithImmediate64 { .. } => Ok(9), // opcode + 8 字节立即数
            ClrInstruction::WithFloat32 { .. } => Ok(5),     // opcode + 4 字节浮点数
            ClrInstruction::WithFloat64 { .. } => Ok(9),     // opcode + 8 字节浮点数
    ...
    fn write_instruction_to_buffer(&self, buffer: &mut Vec<u8>, instruction: &ClrInstruction) -> Result<(), GaiaError> {
        match instruction {
            ...
            ClrInstruction::WithImmediate64 { opcode, value } => match opcode {
                ClrOpcode::LdcI8 => {
                    buffer.push(0x21);
                    buffer.extend_from_slice(&value.to_le_bytes());
                }
                _ => return Err(GaiaError::not_implemented("Unsupported opcode with immediate64")),
            },
            ClrInstruction::WithFloat32 { opcode, value } => match opcode {
                ClrOpcode::LdcR4 => {
                    buffer.push(0x22);
                    buffer.extend_from_slice(&value.to_le_bytes());
                }
                _ => return Err(GaiaError::not_implemented("Unsupported opcode with float32")),
            },
            ClrInstruction::WithFloat64 { opcode, value } => match opcode {
                ClrOpcode::LdcR8 => {
                    buffer.push(0x23);
                    buffer.extend_from_slice(&value.to_le_bytes());
                }
                _ => return Err(GaiaError::not_implemented("Unsupported opcode with float64")),
            },
