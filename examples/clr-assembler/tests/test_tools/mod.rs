...
use clr_assembler::formats::msil::{Item, MsilBuilder, MsilLanguage, MsilRoot};
...
impl MsilExpected {
    ...
    pub fn from_ast(ast: &MsilRoot, file_path: &Path) -> Self {
        ...
        for statement in &ast.items {
            match statement {
                ...
                Item::Assembly(asm) => {
                    assembly_name = Some(asm.name.clone());
                }
                Item::Module(name) => {
                ...
                Item::Class(class) => {
                    ...
                    class_modifiers = vec!["public".to_string()]; // 简化处理
                    extends = None; // 简化处理
    ...
    pub fn validate_ast(&self, ast: &MsilRoot) -> Result<(), String> {
        // 验证语句数量
        if ast.items.len() != self.statement_count {
            return Err(format!("语句数量不匹配: 期望 {}, 实际 {}", self.statement_count, ast.items.len()));
        }
        ...
        for statement in &ast.items {
            match statement {
                ...
                Item::Assembly(asm) => {
                    found_assembly_name = Some(asm.name.clone());
                }
                Item::Module(name) => {
        ...
        if let Some(expected_class_name) = &self.class_name {
            ...
            // oak-msil Class 没有 modifiers 和 extends 字段，跳过验证
...
pub fn compare_msil_file(test_name: &str, file_path: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let ast_result = builder.build(msil_content.as_str(), &[], &mut session);
    ...
    if !Path::new(&json_path).exists() {
        ...
        println!("✓ 解析成功: {} 个语句", ast.items.len());
...
pub fn print_ast_summary(ast: &MsilRoot) {
    println!("解析结果摘要:");
    for (i, statement) in ast.items.iter().enumerate() {
        match statement {
            Item::AssemblyExtern(name) => {
                println!("  {}. 外部程序集: {}", i + 1, name);
            }
            Item::Assembly(asm) => {
                println!("  {}. 程序集: {}", i + 1, asm.name);
            }
            Item::Module(name) => {
                println!("  {}. 模块: {}", i + 1, name);
            }
            Item::Class(class) => {
                println!("  {}. 类: {} (方法数: {})", i + 1, class.name, class.methods.len());
                for (j, method) in class.methods.iter().enumerate() {
                    println!("    {}.{} 方法: {}", i + 1, j + 1, method.name,);
                }
            }
        }
    }
