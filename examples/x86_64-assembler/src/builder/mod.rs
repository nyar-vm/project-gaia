                ...
                | Instruction::Call { target } => {
                    if let Operand::Label(name) = target {
                        if let Some(&label_pos) = labels.get(name) {
                            let mut bytes = assembler.encode(instruction)?;
                            // 简单的短跳转解析 (假设是 2 字节指令，第 2 字节是偏移)
                            if bytes.len() == 2 {
                                let offset = (label_pos as i32) - (current_pos as i32 + 2);
                                bytes[1] = offset as u8;
                            }
                            // 处理近调用 (E8 disp32)
                            else if bytes.len() == 5 && bytes[0] == 0xE8 {
                                let offset = (label_pos as i32) - (current_pos as i32 + 5);
                                bytes[1..5].copy_from_slice(&offset.to_le_bytes());
                            }
                            // 处理近跳转 (E9 disp32)
                            else if bytes.len() == 2 && bytes[0] == 0xEB {
                                let offset = (label_pos as i32) - (current_pos as i32 + 2);
                            ...
                            code.extend(bytes.clone());
                            current_pos += bytes.len();
                        }
                        else {
                            return Err(gaia_types::GaiaError::not_implemented(format!("Label not found: {}", name)));
                        }
                    }
                    else {
                        let bytes = assembler.encode(instruction)?;
