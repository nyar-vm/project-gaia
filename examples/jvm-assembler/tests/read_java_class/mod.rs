...
fn generate_class() -> Result<(), GaiaError> {
    ...
    let javac_creator_output =
        Command::new("javac").current_dir(&base_dir).arg(base_dir.join("JavaClassGenerator.java")).output()?;
...
fn read_class(path: &Path) -> Result<ClassExpected, GaiaError> {
    ...
    // 提取方法信息
    let methods: Vec<MethodInfo> = prog
        .methods
        .iter()
