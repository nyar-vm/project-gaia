...
impl<'config, R: Read + Seek> ClassReader<'config, R> {
    ...
    fn read_program(&self) -> Result<JvmProgram, GaiaError> {
        ...
        let super_class_name =
            if super_class == 0 { None } else { Some(self.resolve_class_name(&constant_pool_entries, super_class)) };
        ...
        for entry in constant_pool_entries.iter() {
            if let Some(value) = entry {
                let cp_entry = crate::program::JvmConstantPoolEntry::Utf8 { value: value.clone() };
    ...
    fn read_class_header(
        &self,
        reader: &mut gaia_types::BinaryReader<impl Read + Seek, BigEndian>,
    ) -> Result<(u16, u16), GaiaError> {
    ...
    fn read_constant_pool(
        &self,
        reader: &mut gaia_types::BinaryReader<impl Read + Seek, BigEndian>,
    ) -> Result<Vec<Option<String>>, GaiaError> {
    ...
    fn read_fields(
        &self,
        reader: &mut gaia_types::BinaryReader<impl Read + Seek, BigEndian>,
        constant_pool_entries: &[Option<String>],
    ) -> Result<Vec<crate::program::JvmField>, GaiaError> {
    ...
    fn read_methods(
        ...
        reader: &mut gaia_types::BinaryReader<impl Read + Seek, BigEndian>,
        constant_pool_entries: &[Option<String>],
    ) -> Result<Vec<crate::program::JvmMethod>, GaiaError> {
    ...
    fn get_string_from_pool(&self, constant_pool_entries: &[Option<String>], index: u16, default: &str) -> String {
        constant_pool_entries.get(index as usize).and_then(|opt| opt.as_ref()).unwrap_or(&default.to_string()).clone()
    }
