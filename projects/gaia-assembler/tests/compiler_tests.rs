...
fn test_basic_compilation() {
    ...
    match result {
        Ok(generated_files) => {
            ...
            assert!(total_size > 0, "编译后的字节码不应为空");
            println!("Hello World 程序编译成功，生成 {} 字节", total_size);
        }
        Err(e) => {
            println!("编译失败: {:?}", e);
...
fn test_multi_function_compilation() {
    ...
    match result {
        Ok(generated_files) => {
            let total_size: usize = generated_files.files.values().map(|v| v.len()).sum();
            assert!(total_size > 0, "多函数程序字节码不应为空");
            println!("多函数程序编译成功，生成 {} 字节", total_size);
        }
        Err(e) => {
            println!("多函数程序编译失败: {:?}", e);
        }
...
fn test_global_variables_compilation() {
    ...
    match result {
        Ok(generated_files) => {
            let total_size: usize = generated_files.files.values().map(|v| v.len()).sum();
            assert!(total_size > 0, "带全局变量的程序字节码不应为空");
            println!("带全局变量的程序编译成功，生成 {} 字节", total_size);
        }
        Err(e) => {
...
fn test_compile_to_all_platforms() {
    ...
    for backend in compiler.backends() {
        ...
        match compiler.compile(&program, &target) {
            Ok(generated_files) => {
                results.push((target, generated_files));
            }
            Err(e) => {
    ...
    for (target, generated_files) in results {
        let total_size: usize = generated_files.files.values().map(|v| v.len()).sum();
        assert!(total_size > 0, "目标 {:?} 的字节码不应为空", target);
        println!("目标 {:?}: {} 字节", target, total_size);
    }
}
...
fn test_compiler_error_handling() {
    ...
    match result {
        Ok(generated_files) => {
            let total_size: usize = generated_files.files.values().map(|v| v.len()).sum();
            println!("空程序编译成功，生成 {} 字节", total_size)
        }
...
fn create_hello_world_program() -> GaiaProgram {
    ...
    GaiaProgram { name: "hello_world".to_string(), functions: vec![main_function], constants: vec![] }
}
...
fn create_multi_function_program() -> GaiaProgram {
    ...
    GaiaProgram { name: "multi_function".to_string(), functions: vec![main_function, helper_function], constants: vec![] }
}
...
fn create_simple_program() -> GaiaProgram {
    ...
    GaiaProgram { name: "simple".to_string(), functions: vec![main_function], constants: vec![] }
}
