...
fn test_function_mapping_management() {
    ...
    let function_mapping = FunctionMapping {
        ...
        platform_mappings: {
            ...
            mappings.insert("msil".to_string(), "TestFunction".to_string());
            mappings
        },
    ...
    let msil_mapping = config_manager.get_function_mapping("test_function", "msil");
        ...
        Ok(()) => {
            ...
            match load_result {
                Ok(()) => {
                    // 验证加载的配置
                    assert_eq!(
                        new_config_manager.get_global_setting("test_setting"),
                        Some("test_value")
                    );
                    println!("配置加载成功");
                }
