...
fn test_all_backends_basic_functionality() {
    ...
    for backend in compiler.backends() {
        ...
        if score > 0.0 {
            ...
            match backend.generate(&program, &config) {
                Ok(generated_files) => {
                    assert!(!generated_files.files.is_empty(), "编译结果不应为空");
                    let total_size: usize = generated_files.files.values().map(|v| v.len()).sum();
                    println!("  编译成功: {} 字节", total_size);
                }
                Err(e) => {
                    println!("  编译失败: {:?}", e);
...
fn test_jvm_backend() {
    ...
    // 查找 JVM 后端
    let jvm_backend = compiler.backends().iter().find(|b| b.name().to_lowercase().contains("jvm")).expect("应该有 JVM 后端");
    ...
    if score > 0.0 {
        // 创建默认配置
        let config = gaia_assembler::config::GaiaConfig::default();
        match jvm_backend.generate(&program, &config) {
            Ok(generated_files) => {
                let total_size: usize = generated_files.files.values().map(|v| v.len()).sum();
                println!("JVM 编译成功: {} 字节", total_size);
                // 可以添加更多 JVM 特定的验证
            }
            Err(e) => {
                println!("JVM 编译失败: {:?}", e);
...
fn test_msil_backend() {
    ...
    // 查找 MSIL 后端
    let msil_backend = compiler
        .backends()
        .iter()
        .find(|b| b.name().to_lowercase().contains("msil_read") || b.name().to_lowercase().contains("clr"))
        .expect("应该有 MSIL 后端");
    ...
    if score > 0.0 {
        // 创建默认配置
        let config = gaia_assembler::config::GaiaConfig::default();
        match msil_backend.generate(&program, &config) {
            Ok(generated_files) => {
                let total_size: usize = generated_files.files.values().map(|v| v.len()).sum();
                println!("MSIL 编译成功: {} 字节", total_size);
                // 可以添加更多 MSIL 特定的验证
            }
            Err(e) => {
                println!("MSIL 编译失败: {:?}", e);
...
fn test_pe_backend() {
    ...
    // 查找 PE 后端
    let pe_backend = compiler.backends().iter().find(|b| b.name().to_lowercase().contains("pe")).expect("应该有 PE 后端");
    ...
    if score > 0.0 {
        // 创建默认配置
        let config = gaia_assembler::config::GaiaConfig::default();
        match pe_backend.generate(&program, &config) {
            Ok(generated_files) => {
                let total_size: usize = generated_files.files.values().map(|v| v.len()).sum();
                println!("PE 编译成功: {} 字节", total_size);
                // 可以添加更多 PE 特定的验证
            }
            Err(e) => {
                println!("PE 编译失败: {:?}", e);
...
fn test_wasi_backend() {
    ...
    // 查找 WASI 后端
    let wasi_backend = compiler.backends().iter().find(|b| b.name().to_lowercase().contains("wasi")).expect("应该有 WASI 后端");
    ...
    if score > 0.0 {
        // 创建默认配置
        let config = gaia_assembler::config::GaiaConfig::default();
        match wasi_backend.generate(&program, &config) {
            Ok(generated_files) => {
                let total_size: usize = generated_files.files.values().map(|v| v.len()).sum();
                println!("WASI 编译成功: {} 字节", total_size);
                // 可以添加更多 WASI 特定的验证
            }
            Err(e) => {
                println!("WASI 编译失败: {:?}", e);
...
fn test_backend_compatibility_scoring() {
    ...
    for backend in compiler.backends() {
        ...
        // 验证评分在有效范围内
        assert!(score >= 0.0 && score <= 1.0, "后端 {} 的兼容性评分 {} 不在有效范围 [0.0, 1.0] 内", backend.name(), score);
    }
...
fn test_backend_error_handling() {
    ...
    // 创建一个可能导致错误的程序
    let invalid_program = GaiaProgram { name: "invalid".to_string(), functions: vec![], constants: vec![] };
    ...
    for backend in compiler.backends() {
        // 创建默认配置
        let config = gaia_assembler::config::GaiaConfig::default();
        let result = backend.generate(&invalid_program, &config);

        // 验证错误处理不会导致崩溃
        match result {
            Ok(generated_files) => {
                let total_size: usize = generated_files.files.values().map(|v| v.len()).sum();
                println!("后端 {} 成功处理了空程序，生成 {} 字节", backend.name(), total_size);
            }
            Err(e) => {
        ...
        if backend.match_score(&target) > 0.0 {
            ...
            match backend.generate(&program, &config) {
                Ok(generated_files) => {
                    ...
                    let total_size: usize = generated_files.files.values().map(|v| v.len()).sum();
                    println!("后端 {} 编译耗时: {:?}, 生成 {} 字节", backend.name(), duration, total_size);
                }
                Err(e) => {
                    let duration = start.elapsed();
                    println!("后端 {} 编译失败，耗时: {:?}, 错误: {:?}", backend.name(), duration, e);
                }
            }
        }
    }
...
fn create_complex_test_program() -> GaiaProgram {
    ...
    GaiaProgram { name: "complex_test_program".to_string(), functions: vec![main_function, helper_function], constants: vec![] }
}
