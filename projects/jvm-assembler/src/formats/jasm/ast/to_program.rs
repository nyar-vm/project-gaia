    ...
    fn convert_instruction(&mut self, instruction: JasmInstruction) -> Result<JvmInstruction> {
        match instruction {
            JasmInstruction::Simple(name) => {
                // 直接根据指令名称创建对应的 JvmInstruction 变体
                match name.as_str() {
            ...
            JasmInstruction::WithArgument { instruction, argument } => {
                ...
                match instruction.as_str() {
                    ...
                    "new" => {
                        Ok(JvmInstruction::New { class_name: argument })
                    }
                    "checkcast" => {
                        Ok(JvmInstruction::Checkcast { class_name: argument })
                    }
                    "instanceof" => {
                        Ok(JvmInstruction::Instanceof { class_name: argument })
                    }
                    "anewarray" => {
                        Ok(JvmInstruction::Anewarray { class_name: argument })
                    }
                    "newarray" => {
                        let atype = argument.parse::<u8>()
