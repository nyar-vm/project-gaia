...
impl JasmToJvmConverter {
    /// 创建新的转换器
    pub fn new() -> Self {
        Self { constant_pool_map: HashMap::new(), constant_pool: JvmConstantPool::new() }
    }

    /// 转换 JASM AST 到 JVM 程序
    ...
    fn convert_instruction(&mut self, instruction: JasmInstruction) -> Result<JvmInstruction> {
        match instruction {
            JasmInstruction::Simple(name) => {
                let opcode = self.parse_opcode(&name)?;
                Ok(JvmInstruction::Simple { opcode })
            }
            JasmInstruction::WithArgument { instruction, argument } => {
                ...
                if argument.starts_with("String ") {
                    ...
                    let symbol = format!("string_{}", string_value);

                    // 添加到常量池
                    self.add_string_constant(string_value);

                    Ok(JvmInstruction::WithConstantPool { opcode, symbol })
                }
                else if let Ok(value) = argument.parse::<i32>() {
                    // 数字常量
                    Ok(JvmInstruction::WithImmediate { opcode, value })
                }
                else {
                    // 其他常量池引用
                    Ok(JvmInstruction::WithConstantPool { opcode, symbol: argument })
                }
            }
            JasmInstruction::MethodCall { instruction, method_ref } => {
                ...
                Ok(JvmInstruction::MethodCall { opcode, class_name, method_name, descriptor })
            }
            JasmInstruction::FieldAccess { instruction, field_ref } => {
                ...
                Ok(JvmInstruction::FieldAccess { opcode, class_name, field_name, descriptor })
            }
        }
    }

    /// 解析操作码
    fn parse_opcode(&self, name: &str) -> Result<JvmOpcode> {
        JvmOpcode::from_str(name).ok_or_else(|| GaiaError::custom_error(format!("Unknown opcode: {}", name)))
    }
    ...
    /// 添加字符串常量到常量池
    fn add_string_constant(&mut self, value: String) {
        let utf8_entry = JvmConstantPoolEntry::Utf8 { value: value.clone() };
        let utf8_index = self.constant_pool.add_entry(utf8_entry);
    ...
    fn add_method_reference(&mut self, class_name: &str, method_name: &str, descriptor: &str) {
        ...
        let class_index = self.constant_pool.add_entry(class_entry);
