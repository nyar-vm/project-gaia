...
在 Nyar 的生态中，Gaia 扮演着“物理执行者”的角色。理解 Gaia 与中端（Chomsky）的职责边界至关重要。

#### 2.1 指令的不变性 (Instruction Invariance)

Gaia 坚持**指令透穿**原则：它不对前端输入的指令流进行任何改变语义或逻辑的变换。这意味着：
* 如果前端传入了 `add x, 0`，Gaia 会忠实地生成加零指令，而不会将其优化掉。
* 如果前端生成了冗余的跳转，Gaia 也会保留这些跳转。
这种设计的目的是让前端（或中端）对最终生成的代码流有绝对的控制权，同时也简化了 Gaia 的实现，使其专注于跨平台的适配。

#### 2.2 深度物理优化 (Deep Physical Optimization)

虽然 Gaia 不改变指令，但它在**物理层、符号层和布局层**拥有极大的优化空间：

* **符号 LTO (Link-Time Optimization)**: Gaia 作为一个智能链接器，能识别跨模块的符号引用。它能精准地从最终二进制中剥离那些“虽然被生成但从未被调用”的函数和数据。
* **冷热布局重排**: 通过分析程序的执行热点（PGO 数据），Gaia 能将热点函数在物理地址上聚合，减小指令缓存（I-Cache）的失效概率。
* **零开销布局转换**: 对于结构体和类，Gaia 能在不改变字段逻辑访问顺序的情况下，通过物理重排（Field Reordering）来挤掉由于对齐产生的空隙。
* **指令编码寻优**: 针对某些指令集（如 x86 或 WASM），Gaia 会在多种等价的编码方式中选择体积最小或执行最快的一种。
...
### 第 4 章：基础层 (The Foundation)

基础层是整个编译器的基石，提供了所有其他组件共享的通用能力。

#### 4.1 `nyar-core`: 通用数据结构
...
##### 5.3.1 第一阶段：高级 MIR (High-Level MIR)
...
##### 5.3.2 第二阶段：MIR 降级遍 (MIR Lowering Passes)
...
### 第 6 章：智能核心：`nyar-interpreter` 与编译时执行
...
#### 6.2 按需单态化 (On-Demand Monomorphization)
