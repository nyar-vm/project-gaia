...
│   └── gaia-types/         # 共享类型定义
├── examples/
│   ├── pe-assembler/       # PE 后端示例
│   ├── clr-assembler/      # CLR 后端示例
│   ├── jvm-assembler/      # JVM 后端示例
│   ├── wasi-assembler/     # WASI 后端示例
│   ├── elf-assembler/      # ELF 后端示例
│   ├── macho-assembler/    # Mach-O 后端示例
│   ├── pyc-assembler/      # Python Bytecode 后端示例
│   └── ...                 # 其他汇编器示例
├── Cargo.toml              # 工作空间配置
└── README.md
```
...
#[tokio::main]
async fn main() {
    let backend = Arc::new(GaiaLanguageServer::new());
    let server = LspServer::new(backend);
    
    server.run(tokio::io::stdin(), tokio::io::stdout()).await.unwrap();
}
```
